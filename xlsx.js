...
function parse_borders(t, styles, themes, opts) {
	// console.log('parse_borders --------------------------')
	styles.Borders = [];
	var border = {}, color = false;
	var pass = false;
	(t[0].match(tagregex)||[]).forEach(function(x,i) {
		var y = parsexmltag(x);
		// console.log('x ---',x)
		switch(strip_ns(y[0])) {
			...
			case '<left/>': break;
			case '<left': border.left = {style:'',color:''}; case '<left>':
				if(y.style) border.left.style = utf8read(y.style); break;
			case '</left>': border.left.color = color; color=null; break;

			/* note: not in spec, appears to be CT_BorderPr */
			case '<right/>': break;
			case '<right': border.right = {style:'',color:''}; case '<right>':
				if(y.style) border.right.style = utf8read(y.style); break;
			case '</right>': border.right.color = color; color=null; break;

			/* 18.8.43 top CT_BorderPr */
			case '<top/>': break;
			case '<top': border.top = {style:'',color:''}; case '<top>':
				if(y.style) border.top.style = utf8read(y.style); break;
			case '</top>': border.top.color = color; color=null; break;

			/* 18.8.6 bottom CT_BorderPr */
			case '<bottom/>': break;
			case '<bottom': border.bottom = {style:'',color:''}; case '<bottom>':
				if(y.style) border.bottom.style = utf8read(y.style); break;
			case '</bottom>': border.bottom.color = color; color=null; break;

			// /* 18.8.13 diagonal CT_BorderPr */
			// case '<diagonal': case '<diagonal>': 
			// case '<diagonal/>': 
			// border.diagonal = {style:'',color:'000000',diagonalUp:true,diagonalDown:true}; break;
			// case '</diagonal>': break;
...
return function parse_sty_xml(data, themes, opts) {
	...
	var t;
	// console.log('(t=data.match(bordersRegex))',(data.match(bordersRegex)))
...
function safe_format(p, fmtid, fillid, opts, themes, styles, fontid, borderid,cf) {
	// console.log('p',p,'fmrid',fmtid,'fillid',fillid)
	if(fillid == 2 && p.t != 'z'){
		// console.log('p --',p,table_fmt[fmtid])
	}
	try {
	...
	// 		if(opts.WTF) p.s.bgColor.raw_rgb = themes.themeElements.clrScheme[pss.bgColor.theme].rgb;
	// 	}
	// 	if(pss.patternType){
	// 		if(pss.fgColor.rgb){
...
function parse_ws_xml_pageSetup(s,pagesetup){
	s['!pageSetup'] = {}
	pagesetup[0].match(pagesetuptagregex).map((el)=>{
		let keyValue = el.split('=')
		s['!pageSetup'] = {
			...s['!pageSetup'],
			[keyValue[0]]: keyValue[1].replace(/"/g,'')
		}
	})
	// s['!pageSetup'] = pagesetup[0]
}
function parse_ws_xml_colBreaks(s, colbreaks){
	var brkregex = /<(?:\w:)?brk[^>]*\/>/g;
	var colBreaksregex = /<(?:\w:)?colBreaks[^>]*>/;
	s['!colBreaks'] = {brk:[],colBreaks:{}}
	colbreaks[0].match(brkregex).map((el) => {
		let obj = {}
		el.match(pagesetuptagregex).map((el)=>{
			let keyValue = el.split('=')
			obj = {
				...obj,
				[keyValue[0]]: keyValue[1].replace(/"/g,'')
		...
		s['!colBreaks'].brk.push(obj)
	})
	colbreaks[0].match(colBreaksregex).map((el)=>{
		el.match(pagesetuptagregex).map((elm) => {
			let keyValue = elm.split('=')
			s['!colBreaks'].colBreaks = {
				...s['!colBreaks'].colBreaks,
				[keyValue[0]]: keyValue[1].replace(/"/g,'')
			}
		})
...
return function parse_ws_xml_data(sdata, s, opts, guess, themes, styles) {
	// console.log('parse_ws_xml_data')
	var ri = 0, x = "", cells = [], cref = [], idx=0, i=0, cc=0, d="", p;
	var tag, tagr = 0, tagc = 0;
	var sstr, ftag;
	var fmtid = 0, fillid = 0, fontid = 0, borderid = 0;
...
function parse_workbook(blob, options) {
	...
	var process_cell_style = function pcs(cell, line, options) {
		// console.log('cell',cell,'line',line)
...
function safe_parse_sheet(zip, path, relsPath, sheet, idx, sheetRels, sheets, stype, opts, wb, themes, styles) {
	// console.log('safe_parse_sheet','zip',zip, path, relsPath,sheet)
	try {
...
function read_cfb(cfb, opts) {
	// console.log('read_cfd')
	if(CFB.find(cfb, "EncryptedPackage")) return parse_xlsxcfb(cfb, opts);
	return parse_xlscfb(cfb, opts);
}

function read_zip(data, opts) {
	// console.log('read_zip')
	var zip, d = data;
	var o = opts||{};
